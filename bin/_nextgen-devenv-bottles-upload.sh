...
brew tap nextgen-devenv
cd $(brew tap-info --json nextgen-devenv | jq -r '.[].path' | perl -pe 's/\+/\ /g;' -e 's/%(..)/chr(hex($1))/eg;')
#git stash
#git pull
...
for FORMULA in $FORMULAS; do
    ...
    for jsonfile in ./*.json; do
        ...
        if ! [[ -z $JSON_FORMULA_NAME ]]; then
            mergedfile=$(jq -r '.["nextgen-devenv/'$JSON_FORMULA_NAME'"].formula.name + "-" + ."nextgen-devenv/'$JSON_FORMULA_NAME'".formula.pkg_version + ".json"' "$jsonfile" | perl -pe 's/\+/\ /g;' -e 's/%(..)/chr(hex($1))/eg;')
            set -x
            ...
            done < <(jq -r '."nextgen-devenv/'$JSON_FORMULA_NAME'".bottle.tags[].filename' "$jsonfile" | perl -pe 's/\+/\ /g;' -e 's/%(..)/chr(hex($1))/eg;')
            set +x
            s3cmd info "s3://homebrew-bottles/$FORMULA/$mergedfile" >/dev/null && {
                ...
                jq -s  '.[1]."nextgen-devenv/'$JSON_FORMULA_NAME'".bottle.tags = .[0]."nextgen-devenv/'$JSON_FORMULA_NAME'".bottle.tags * .[1]."nextgen-devenv/'$JSON_FORMULA_NAME'".bottle.tags | .[1]' "$mergedfile".src "$jsonfile" > "$mergedfile"
