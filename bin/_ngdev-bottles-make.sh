...
do
    ...
    if [[ -n "$FORMULAS" ]]; then
        for FORMULA in $FORMULAS; do
            ...
            if [[ -n $REBUILD ]]; then
                brew list | grep $FORMULA && brew uninstall --force --ignore-dependencies $FORMULA $(brew deps --full --direct $FORMULA | grep "${TAP_NAME}")
    ...
    for FORMULA in $FORMULAS; do
        ...
        if ! [[ -d ${HOME}/.bottles/${FORMULA//"$TAP_NAME_PREFIX"/}.bottle ]] || ! grep "$FORMULA$" /tmp/.${TAP_SUBDIR}_bottles_created_${FORMULAS_MD5}.tmp; then
            ...
            [[ "true" == $(brew info --json=v1 $FORMULA | jq '.[0].installed[0].built_as_bottle') ]] || {
                ...
                brew list | grep $FORMULA && brew uninstall --force --ignore-dependencies $FORMULA
            }
